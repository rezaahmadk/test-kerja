...
<html>
<head>
	<title>Mengubah Data Jabatan Pegawai</title>
...
<?php 
	...
	if(isset($_POST['ubah'])){
		$update = new LibraryJabatanPegawai();
		$JabatanPegawai = $_POST['JabatanPegawai'];
		$update->UpdateJabatanPegawai($idJabatanPegawai, $JabatanPegawai);
	}
	else{
		$read = new LibraryJabatanPegawai();
		$result = $read->ReadUpdateJabatanPegawai($idJabatanPegawai);
    ...
    <main role="main" class="container-fluid main mt-2">
		<div class="card">
	  		<div class="card-header">
		    	<center><strong>Mengubah Data Jabatan</strong></center>
		  	</div>
		  	<div class="card-body">
			    <form method="post" action="updatejabatan.php?id=<?php echo $idJabatanPegawai?>">
	        		<div class="form-group">     
	        			<label for="JabatanPegawai">Jabatan Pegawai</label> 	
					    <input name="JabatanPegawai" class="form-control" type="text" placeholder="Masukkan Jabatan Pegawai" autocomplete="off" required="" value="<?php echo !empty($JabatanPegawai)?$JabatanPegawai:'';?>">
					</div>
					<center>
